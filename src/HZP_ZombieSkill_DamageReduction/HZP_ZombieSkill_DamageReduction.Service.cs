...
public sealed class HZP_ZombieSkill_DamageReduction_Service
{
    private readonly ISwiftlyCore _core;
    private readonly HZP_ZombieSkill_DamageReduction_Helpers _helpers;
    private readonly HZP_ZombieSkill_DamageReduction_Config _config;
    private readonly HZP_ZombieSkill_DamageReduction_Globals _globals;

    public HZP_ZombieSkill_DamageReduction_Service(
        ISwiftlyCore core,
        HZP_ZombieSkill_DamageReduction_Helpers helpers,
        IOptions<HZP_ZombieSkill_DamageReduction_Config> config,
    ...
    {
        _helpers.TryHandleDamageReduction(@event);
    }
    ...
    private void OnButtonChange(IOnClientKeyStateChangedEvent @event)
    {
        ...
        if (state.IsActive)
        {
            player.SendCenter(_helpers.T(player, "DamageReductionAlreadyActive"));
            return;
        }
        ...
        if (currentTime < state.CooldownEndTime)
        {
            ...
            player.SendCenter(_helpers.T(player, "DamageReductionCooldown", remaining));
            return;
        }

        _helpers.GiveDamageReduction(player, group);
    }

    private DamageReductionSkillGroup? GetEnabledGroup(IPlayer player)
    {
        var zpApi = HZP_ZombieSkill_DamageReduction.ZpApi;
        if (zpApi == null)
            return null;
